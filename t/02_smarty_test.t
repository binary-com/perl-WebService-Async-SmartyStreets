...
subtest "Call SmartyStreets" => sub {
    ...
    is( $addr->status_at_least('verified'),
        '', "Verification score is correct" );

    # Check if address accuracy level check is correct
    is( $addr->accuracy_at_least('locality'),
        1, "Accuracy checking is correct" );
    is( $addr->accuracy_at_least('administrativearea'),
        1, "Accuracy checking is correct" );
    is( $addr->accuracy_at_least('deliverypoint'),
        '', "Accuracy checking is correct" );
    ...
    is( $addr->status_at_least('partial'),
        '', "Verification score is correct for empty status" );
...
# get_decoded_data returns the first element of the data, we wanted to test empty response [], so `undef` would be the result of that operation.
    ...
    $addr = $ss->verify(%data)->get();
    is( $addr->status_at_least('partial'),
        '', "Verification score is correct for empty http response" );
};
